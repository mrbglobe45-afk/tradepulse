...
import { Plus, TrendingUp, TrendingDown, ListPlus, Tags, Banknote, Settings, Trash2, Trophy, BarChart3, Brain } from "lucide-react";
import { AreaChart, Area, XAxis, YAxis, ResponsiveContainer, Tooltip, Line, BarChart, Bar, Cell, ReferenceLine, CartesianGrid } from "recharts";
...
function fmtMoneyRight(val: number, code: string) {
  const sym = CURRENCIES[code as keyof typeof CURRENCIES] || "";
...
function densifyAtZero(points: {date?: string; x: number; value: number}[]) {
...
export default function TradePulseApp() {
  ...
  const segments = useMemo(() => {
    ...
    const segs: {sign: 1|-1; data: typeof pts}[] = [];
    let seg = [pts[0]];
    let sign: 1 | -1 | 0 = (Math.sign(pts[0].value) as any) || 0;
    for (let i = 1; i < pts.length; i++) {
      const v: number = pts[i].value as number;
  ...
  const yDomain = useMemo<[number, number]>(() => {
    ...
    const vals = equityR.map((p: any) => p.value as number);
    const min = Math.min(...vals), max = Math.max(...vals);
    const pad = Math.max(0.5, Math.max(1e-6, max - min) * 0.10);
    return [min - pad, max + pad];
  }, [equityR]);

  const average = (arr: number[]) => arr.length ? arr.reduce((a: number, b: number) => a + b, 0) / arr.length : 0;
  const wins = Rs.filter((r: number) => r > 0).length;
  const winRate = Rs.length ? (wins / Rs.length) * 100 : 0;
  const avgR = average(Rs);
  const { bestWin, bestLoss } = useMemo(() => longestStreaks(Rs), [Rs]);

  const currencySymbol = CURRENCIES[settings.currencyCode as keyof typeof CURRENCIES];
  ...
  const totalR = equityR.length ? (equityR[equityR.length - 1].value as number) : 0;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card className="card-plain rounded-2xl">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Select value={settings.currencyCode} onValueChange={(v) => setSettings(s => ({ ...s, currencyCode: v }))}>
          ...
          <CardContent className="h-[26rem] md:h-[30rem]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={equityR} margin={{ left: 12, right: 12 }}>
                ...
                {segments.map((seg: {sign: 1|-1; data: any[]}, i: number) => (
                  <Area
                    key={i}
                    type="linear"
                    data={seg.data}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <MiniBarCard title={`Profit par symbole (${unitLabel})`} data={aggregate(byKey('symbol', filtered as any[], Rs, valueFromR)).slice(0,8)} unitLabel={unitLabel} />
          <MiniBarCard title={`Profit par setup (${unitLabel})`}  data={aggregate(byKey('setup',  filtered as any[], Rs, valueFromR)).slice(0,8)} unitLabel={unitLabel} />
          <MiniBarCard title={`Profit par direction (${unitLabel})`} data={aggregate(byKey('direction',filtered as any[], Rs, valueFromR))} unitLabel={unitLabel} />
        </div>
          ...
          <DialogContent className="max-w-md bg-gradient-to-b from-[#141421] to-[#0b0b10] text-white border border-white/20 rounded-3xl p-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-white/70 block">Capital de départ ({CURRENCIES[settings.currencyCode as keyof typeof CURRENCIES]})</Label>
                <Input
                  type="number"
                  inputMode="decimal"
              ...
              <div className="space-y-2">
                <Label className="text-white/70 block">Valeur d’un R ({CURRENCIES[settings.currencyCode as keyof typeof CURRENCIES]})</Label>
                <Input
                  type="number"
                  inputMode="decimal"
                  placeholder="ex: 100"
...
function MiniBarCard({ title, data, unitLabel }: {title:string; data:{name:string; value:number}[]; unitLabel:string}) {
  return (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ left: 8, right: 8, top: 8, bottom: 0 }}>
            ...
            <Tooltip
              ...
              content={(ctx: any) => {
                const { active, payload, label } = ctx || {};
                if (!active || !payload || !payload.length) return null;
...
const aggregate = (rows: {k:string;v:number}[]) =>
  Array.from(
    rows.reduce((m: Map<string,number>, {k, v}: {k:string; v:number}) => m.set(k, (m.get(k) || 0) + v), new Map<string,number>()),
